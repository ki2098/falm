...
__global__ static void fscala_norm2sq_kernel(FieldCp<double> &a, double *partial_sum, Dom dom, Dom mapper) {
...
static double fscala_norm2sq(Field<double> &a, Dom &dom) {
    assert(a._col == 1);

    dim3 &sz = dom._size;
    const unsigned int g = guide;
    Dom mapper(
        dim3(dom._size.x - 2 * g, dom._size.y - 2 * g, dom._size.z - 2 * g), 
        dim3(g, g, g)
    );
    ...
    fscala_norm2sq_kernel<<<n_blocks, n_threads>>>(*(a._dd), partial_sum_dev, dom, mapper);
...
__global__ static void fscala_sum_kernel(FieldCp<double> &a, double *partial_sum, Dom dom, Dom mapper) {
    __shared__ double cache[n_threads];
    unsigned int stride = FALMUtil::get_global_size();
    double temp_sum = 0;
    for (unsigned int idx = FALMUtil::get_global_idx(); idx < mapper._num; idx += stride) {
...
static double fscala_sum(Field<double> &a, Dom &dom) {
    assert(a._col == 1);
    ...
    fscala_sum_kernel<<<n_blocks, n_threads>>>(*(a._dd), partial_sum_dev, dom, mapper);
...
__global__ static void fscala_zero_avg_kernel(FieldCp<double> &a, Dom dom, Dom mapper, double avg) {
    unsigned int stride = FALMUtil::get_global_size();
    for (unsigned int idx = FALMUtil::get_global_idx(); idx < mapper._num; idx += stride) {
        unsigned int ii, ij, ik;
        FALMUtil::d123(idx, ii, ij, ik, mapper._size);
...
static void fscala_zero_avg(Field<double> &a, Dom &dom, Dom &global, int mpi_size, int mpi_rank) {
    ...
    double avg = sum / ginum;
    Dom mapper(
        dim3(dom._size.x - 2 * g, dom._size.y - 2 * g, dom._size.z - 2 * g), 
        dim3(g, g, g)
    );
    fscala_zero_avg_kernel<<<n_blocks, n_threads>>>(*(a._dd), dom, mapper, avg);
}
