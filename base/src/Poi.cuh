...
static void poisson_sor(Field<double> &a, Field<double> &x, Field<double> &b, Field<double> &r, double omega, int maxit, Dom &dom, Dom &global, int mpi_size, int mpi_rank, MPI_Request *req, LS_State &state, double &kernel_time, double &comm_time) {
    ...
    for (state.it = 0; state.it < maxit;) {
        color = 0;
        if (mpi_size > 1) {
            int buflen = sz.y * sz.z;
            if (mpi_rank == 0) {
                MPI_Isend(&(x._hd._arr[FALMUtil::d321(sz.x-g-1,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank+1, 1, MPI_COMM_WORLD, &req[0]);
                MPI_Irecv(&(x._hd._arr[FALMUtil::d321(sz.x-g  ,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank+1, 0, MPI_COMM_WORLD, &req[1]);
            } else if (mpi_rank == mpi_size - 1) {
                MPI_Isend(&(x._hd._arr[FALMUtil::d321(      g  ,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank-1, 0, MPI_COMM_WORLD, &req[0]);
                MPI_Irecv(&(x._hd._arr[FALMUtil::d321(      g-1,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank-1, 1, MPI_COMM_WORLD, &req[1]);
            } else {
                MPI_Isend(&(x._hd._arr[FALMUtil::d321(sz.x-g-1,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank+1, 1, MPI_COMM_WORLD, &req[0]);
                MPI_Irecv(&(x._hd._arr[FALMUtil::d321(sz.x-g  ,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank+1, 0, MPI_COMM_WORLD, &req[1]);
                MPI_Isend(&(x._hd._arr[FALMUtil::d321(      g  ,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank-1, 0, MPI_COMM_WORLD, &req[2]);
                MPI_Irecv(&(x._hd._arr[FALMUtil::d321(      g-1,0,0,sz)]), buflen, MPI_DOUBLE, mpi_rank-1, 1, MPI_COMM_WORLD, &req[3]);
            }
        }
        i1 = (mpi_rank == 0           )? (       g) : (       g + 1);
        i2 = (mpi_rank == mpi_size - 1)? (sz.x - g) : (sz.x - g - 1);
        map.set(
            dim3(i2 - i1, sz.y - 2 * g, sz.z - 2 * g),
            dim3(i1, g, g)
        );
        t0 = MPI_Wtime();
        ...
        map.set(
            dim3(sz.x - 2 * g, sz.y - 2 * g, sz.y - 2 * g),
            dim3(g, g, g)
        );
        res_kernel<<<n_blocks, n_threads>>>(*(a._dd), *(x._dd), *(b._dd), *(r._dd), dom, map);
        err = fscala_norm2sq(r, dom);
        if (mpi_size > 1) {
            MPI_Allreduce(MPI_IN_PLACE, &err, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        }
        unsigned int ginum = (global._size.x - 2 * g) * (global._size.y - 2 * g) * (global._size.z - 2 * g);
        state.re = sqrt(err) / ginum;
        state.it ++;
