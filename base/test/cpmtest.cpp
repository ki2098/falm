...
int main(int argc, char **argv) {
    ...
    if (cpm.size > 1) {
        ...
        CPM_Barrier(MPI_COMM_WORLD);
        cpm.dev_IExchange6ColoredFace(x.dev.ptr, process, Color::Black, 2, 0, buffer, req);
        cpm.Wait6Face(req);
        cpm.dev_PostExchange6ColoredFace(x.dev.ptr, process, Color::Black, buffer, req);
    }
    ...
    if (cpm.size > 1) {
        ...
        CPM_Barrier(MPI_COMM_WORLD);
        cpm.dev_IExchange6ColoredFace(x.dev.ptr, process, Color::Red, 2, 0, buffer, req);
        cpm.Wait6Face(req);
        cpm.dev_PostExchange6ColoredFace(x.dev.ptr, process, Color::Red, buffer, req);
    }
    x.sync(MCpType::Dev2Hst);
    for (int i = 0; i < cpm.size; i ++) {
        if (cpm.rank == i) {
